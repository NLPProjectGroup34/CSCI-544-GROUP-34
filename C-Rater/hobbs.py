...
def resolve_pronoun(trees):
    ...
    r = ["Himself", "himself", "Herself", "herself",
         "Itself", "itself", "Themselves", "themselves"]
    #trees = [Tree.fromstring(s) for s in parsedTree]
    pronouns = traverseTree(trees[-1], [], False)
    values = {}
    for pro in pronouns:
        pos = get_pos(trees[-1], pro)
        pos = pos[:-1]
        ...
        if pro in p:
            tree, pos = hobbs(trees, pos)
            val = ""
            for s in traverseTree(tree[pos], [], True):
                val += s + " "
...
def traverseTree(tree, list, string):
    for subtree in tree:
        if type(subtree) == nltk.tree.Tree:
            if subtree.label() == "PRP" and not string:
                for s in subtree:
                    if type(s) == str:
                        list.append(s)
            traverseTree(subtree, list, string)
        elif type(subtree) == str and string:
            list.append(subtree)
    return list
...
def generateSentence(values, tree):
    ...
    for leafPos in tree[0].treepositions('leaves'):
        sentence += tree[0][leafPos] + " "
    sentence = sentence.strip() + ". "
    for leafPos in tree[1].treepositions('leaves'):
        if tree[1][leafPos] in values.keys():
            sentence += values[tree[1][leafPos]][0] + " "
        else:
            sentence += tree[1][leafPos] + " "
    sentence = sentence.strip() + "."
