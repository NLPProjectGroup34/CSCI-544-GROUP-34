...
def calculate_accuracy(scores_given_by_professor, scores_scale):
    accuracy_count = 0
    count_correctly_predicted_grade = 0
    count_incorrectly_predicted_grade = 0
    for prof_score_key in scores_given_by_professor.keys():
        ...
        if (float(m_s) >= (float(p_s) -1.0)) and (float(m_s) <= (float(p_s) +1.0)):
            accuracy_count += 1
            count_correctly_predicted_grade +=1
        else:
            count_incorrectly_predicted_grade +=1
    return(accuracy_count,count_correctly_predicted_grade, count_incorrectly_predicted_grade )
result = calculate_accuracy(scores_given_by_professor,scores_scale)

final_accuracy = ((result[0]/len(scores_given_by_lsa))*100)
precision = ((result[1]/len(scores_given_by_lsa))*100)
recall = ((result[2]/len(scores_given_by_lsa))*100)
